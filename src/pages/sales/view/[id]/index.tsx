            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="User Sale Customer Id Touser"
                    text={
                      <Link as={NextLink} href={`/users/view/${data?.user_sale_customer_idTouser?.id}`}>
                        {data?.user_sale_customer_idTouser?.email}
                      </Link>
                    }
                  />
                )}
                {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="User Sale Sales Associate Id Touser"
                    text={
                      <Link as={NextLink} href={`/users/view/${data?.user_sale_sales_associate_idTouser?.id}`}>
                        {data?.user_sale_sales_associate_idTouser?.email}
                      </Link>
                    }
                  />
                )}
